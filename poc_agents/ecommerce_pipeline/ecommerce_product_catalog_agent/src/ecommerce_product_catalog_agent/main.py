...
app = FastAPI(
    ...
    description="Retrieves product details by ID or search term.",
    version="0.1.0"
)
...
try:
    ...
    agent_instance = ProductCatalogAgent()
...
@app.get("/", tags=["Status"])
async def read_root():
    return {"message": "Product Catalog Agent running"}
...
logger.info("Product Catalog Agent application initialized.")
...
if __name__ == "__main__":
    ...
    uvicorn.run("ecommerce_product_catalog_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
