...
from .agent import RecommendationEngineAgent
...
try:
    ...
    agent_instance = RecommendationEngineAgent()
...
@app.get("/", tags=["Status"])
async def read_root():
    return {"message": "Recommendation Engine Agent running"}
...
logger.info("Recommendation Engine Agent application initialized.")
...
if __name__ == "__main__":
    ...
    uvicorn.run("ecommerce_recommendation_engine_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
