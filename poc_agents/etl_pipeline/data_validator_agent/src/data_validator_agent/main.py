...
from .agent import DataValidatorAgent
...
app = FastAPI(
    ...
    description="Validates transformed data artifact fetched from DB.",
    version="0.1.0"
)
...
try:
    ...
    agent_instance = DataValidatorAgent()
...
# --- Root Endpoint ---
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Data Validator Agent running"}
...
logger.info("Data Validator Agent application initialized.")
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Uvicorn server for Data Validator Agent on host 0.0.0.0, port {port}")
    uvicorn.run("data_validator_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
