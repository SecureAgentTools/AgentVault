...
from .agent import DataExtractorAgent
...
app = FastAPI(
    ...
    description="Extracts data from CSV and stores raw artifact in DB.",
    version="0.1.0"
)
...
try:
    ...
    agent_instance = DataExtractorAgent()
...
# --- Root Endpoint ---
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Data Extractor Agent running"}
...
async def shutdown_event():
    if hasattr(agent_instance, 'close') and callable(agent_instance.close):
        ...
        await agent_instance.close() # Should close the DB pool
        logger.info("Agent resources closed.")

logger.info("Data Extractor Agent application initialized.")
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Uvicorn server for Data Extractor Agent on host 0.0.0.0, port {port}")
    uvicorn.run("data_extractor_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
