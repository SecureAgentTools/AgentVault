...
# --- FastAPI App Setup ---
app = FastAPI(
    title="Data Transformer Agent",
    description="Transforms raw data artifact fetched from DB.",
    version="0.1.0"
)
...
try:
    ...
    agent_instance = DataTransformerAgent()
...
# --- Root Endpoint ---
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Data Transformer Agent running"}
...
logger.info("Data Transformer Agent application initialized.")
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Uvicorn server for Data Transformer Agent on host 0.0.0.0, port {port}")
    uvicorn.run("data_transformer_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
