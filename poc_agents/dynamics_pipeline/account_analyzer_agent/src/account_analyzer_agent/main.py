...
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Account Health Analyzer Agent running"}
...
logger.info("Account Health Analyzer Agent application initialized.")

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8052))
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for Account Health Analyzer Agent on host 0.0.0.0, port {port}")
    uvicorn.run("account_analyzer_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
