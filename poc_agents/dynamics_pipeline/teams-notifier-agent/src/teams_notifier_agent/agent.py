...
AGENT_ID = "local-poc/teams-notifier"

# --- Helper function for SSE Formatting ---
# (Copied - standard utility)
...
class TeamsNotifierAgent(BaseA2AAgent):
    """Agent to mock sending Teams notifications by logging."""
    def __init__(self):
        super().__init__(agent_metadata={"name": "Teams Notifier Agent (Mock)"})
        self.task_store: Optional[Any] = None
        self.logger = logger # Assign logger
        logger.info("Teams Notifier Agent initialized.")
    ...
    async def _mock_send_notification(self, input_data: SendNotificationInput) -> SendNotificationOutput:
        ...
        try:
            ...
            self.logger.info(f"MOCK TEAMS: Sending to target '{input_data.target}' message: '{input_data.message_text}'")
            output.success = True
            output.message = "Mock Teams notification logged successfully."
        except Exception as e:
            self.logger.exception(f"Unexpected error during mock Teams notification logging: {e}")
    ...
    async def handle_task_send(self, task_id: Optional[str], message: Message, background_tasks: Optional[BackgroundTasks] = None) -> str:
        ...
        if task_id: raise AgentProcessingError(f"Teams notifier agent does not support continuing task {task_id}")
        new_task_id = f"teams-notify-{uuid.uuid4().hex[:8]}"
        self.logger.info(f"Task {new_task_id}: Received Teams notification request.")
    ...
    async def process_task(self, task_id: str, content: Dict[str, Any]):
        ...
        self.logger.info(f"Task {task_id}: Background processing started for Teams notification.")
        final_state = TaskStateEnum.FAILED
        error_message = "Failed mock Teams notification."
        ...
        try:
            try:
                ...
                raise AgentProcessingError(f"Invalid input for Teams notification: {val_err}")

            target = input_data.target
            self.logger.info(f"Task {task_id}: Processing mock Teams notification for target '{target}'.")
            ...
            if output_data.success:
                ...
                error_message = None
                self.logger.info(f"Task {task_id}: Mock Teams notification processed successfully.")
            else:
                final_state = TaskStateEnum.FAILED # Keep FAILED if mock operation failed internally
                error_message = output_data.message
                self.logger.error(f"Task {task_id}: Mock Teams notification processing failed: {output_data.message}")
            ...
            self.logger.exception(f"Task {task_id}: Unexpected error during Teams notification process: {e}")
    ...
    async def close(self):
        # (Standard implementation)
        self.logger.info("Teams Notifier Agent closed.")
