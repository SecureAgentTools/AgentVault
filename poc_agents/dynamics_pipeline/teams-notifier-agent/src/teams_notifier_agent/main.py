...
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Teams Notifier Agent running"}
...
logger.info("Teams Notifier Agent application initialized.")

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8058))
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for Teams Notifier Agent on host 0.0.0.0, port {port}")
    uvicorn.run("teams_notifier_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
