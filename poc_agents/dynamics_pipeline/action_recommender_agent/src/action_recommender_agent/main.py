...
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Action Recommendation Agent running"}
...
logger.info("Action Recommendation Agent application initialized.")

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8054))
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for Action Recommendation Agent on host 0.0.0.0, port {port}")
    uvicorn.run("action_recommender_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
