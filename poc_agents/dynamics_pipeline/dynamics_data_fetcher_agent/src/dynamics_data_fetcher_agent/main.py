...
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Dynamics Data Fetcher Agent running"}
...
logger.info("Dynamics Data Fetcher Agent application initialized.")

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8050))
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for Dynamics Data Fetcher Agent on host 0.0.0.0, port {port}")
    uvicorn.run("dynamics_data_fetcher_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
