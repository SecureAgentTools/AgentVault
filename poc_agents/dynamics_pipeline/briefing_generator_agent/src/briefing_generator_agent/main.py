...
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Briefing Generator Agent running"}
...
logger.info("Briefing Generator Agent application initialized.")

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8053))
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for Briefing Generator Agent on host 0.0.0.0, port {port}")
    uvicorn.run("briefing_generator_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
