...
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "Dynamics Task Creator Agent running"}
...
logger.info("Dynamics Task Creator Agent application initialized.")

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8056))
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for Dynamics Task Creator Agent on host 0.0.0.0, port {port}")
    uvicorn.run("dynamics_task_creator_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
