...
@app.get("/", tags=["Status"])
async def read_root(): return {"message": "External Data Enrichment Agent running"}
...
async def shutdown_event():
    if hasattr(agent_instance, 'close') and callable(agent_instance.close):
        logger.info("Closing agent resources (DB pool)..."); await agent_instance.close(); logger.info("Agent resources closed.")

logger.info("External Data Enrichment Agent application initialized.")

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8051))
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for External Data Enrichment Agent on host 0.0.0.0, port {port}")
    uvicorn.run("external_data_enricher_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
