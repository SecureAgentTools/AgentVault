...
# Create a single router with proper configuration
# This version handles the BackgroundTasks injection correctly
...
# Add a direct RPC endpoint for backward compatibility
@app.post("/rpc")
async def handle_rpc(request: dict, background_tasks: BackgroundTasks):
    """Direct JSON-RPC endpoint that ensures BackgroundTasks is provided."""
    ...
    # Handle agent.handle_task_send specially
    if method == "agent.handle_task_send":
        try:
            # Explicitly pass background_tasks to the method
            task_id = await agent.handle_task_send(
                task_id=params.get("task_id"),
                message=params.get("message"),
                background_tasks=background_tasks
            )
            return {"jsonrpc": "2.0", "result": {"id": task_id}, "id": req_id}
        except Exception as e:
            logger.exception(f"Error in RPC handler for {method}: {e}")
            return {"jsonrpc": "2.0", "error": {"code": -32000, "message": str(e)}, "id": req_id}
            
    # For other methods, use standard routing
    return {"jsonrpc": "2.0", "error": {"code": -32601, "message": f"Method {method} not implemented"}, "id": req_id}
