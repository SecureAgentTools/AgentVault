...
app = FastAPI(title="SecOps MCP Tool Proxy Agent", version="0.1.0")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])

# Agent Card Path Logic
CARD_PATH_STR = os.environ.get("AGENT_CARD_PATH", "/app/agent-card.json")
...
@app.get("/", tags=["Status"])
async def read_root():
    return {"message": "SecOps MCP Tool Proxy Agent running"}
...
async def shutdown_event():
    if hasattr(agent_instance, 'close') and callable(agent_instance.close):
        logger.info("Closing agent resources (HTTP client)...")
        await agent_instance.close()
        logger.info("Agent resources closed.")

logger.info("SecOps MCP Tool Proxy Agent application initialized.")

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8069))
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for SecOps MCP Tool Proxy Agent on host 0.0.0.0, port {port}")
