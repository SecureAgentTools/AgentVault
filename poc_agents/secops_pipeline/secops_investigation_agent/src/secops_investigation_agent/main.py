...
# SDK Imports - These are critical for the A2A protocol
try:
...
from .agent import SimpleInvestigationAgent
...
# --- FastAPI App Setup ---
app = FastAPI(title="SecOps Investigation Agent", version="0.1.0")
...
if _SDK_AVAILABLE and _AGENT_LOGIC_AVAILABLE:
    try:
        # Create a task store for the agent
        task_store_instance = InMemoryTaskStore()
        
        # Initialize the simple agent
        agent_instance = SimpleInvestigationAgent(task_store=task_store_instance)
        logger.info("Initialized SimpleInvestigationAgent.")

        # --- A2A Router Setup - This is CRUCIAL for the A2A protocol ---
        router_dependencies = []
        ...
        logger.info("A2A router included. Should handle 'tasks/send' method correctly.")
    ...
    logger.critical("AgentVault SDK or agent logic not available. Cannot initialize agent.")
    @app.get("/")
    async def disabled_root(): 
        return {"error": "Agent failed to initialize due to missing dependencies."}
    @app.post("/a2a")
    async def disabled_a2a(): 
        raise HTTPException(status_code=503, detail="Agent service unavailable - initialization failed.")

# --- Standard Endpoints ---
AGENT_ID = "SecOps Investigation Agent"
...
async def shutdown_event():
    logger.info("SecOps Investigation Agent shutting down...")
...
if __name__ == "__main__":
    ...
    uvicorn.run("secops_investigation_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=True)
