...
class MCPToolProxyAgent(BaseA2AAgent):
    """Agent to proxy A2A requests to underlying MCP tool servers."""
    def __init__(self):
        super().__init__(agent_metadata={"name": "MCP Tool Proxy Agent"})
        ...
        logger.info(f"MCP Tool Proxy Agent initialized. Map: {self.mcp_server_map}")
    ...
    def _get_mcp_server_url(self, target_id_or_tool_name: str) -> Optional[str]:
        ...
        # 2. Prefix match based on tool_name (e.g., "filesystem" for "filesystem.readFile")
        if '.' in target_id_or_tool_name:
            prefix = target_id_or_tool_name.split('.', 1) # Corrected split index access
    ...
    async def _call_mcp_server(self, mcp_base_url: str, tool_name: str, arguments: Dict[str, Any]) -> McpToolExecOutput:
        ...
        """
        output = McpToolExecOutput(success=False, mcp_result=None, error=None) # Init error field
        request_id = f"mcp-req-{uuid.uuid4().hex[:8]}"

        mcp_request_payload = {
            "jsonrpc": "2.0",
            "id": request_id,
            "method": tool_name, # Use the specific tool name (e.g., "code.runPython", "filesystem.readFile")
            "params": arguments   # Pass the arguments directly as params
        }

        # --- MODIFIED TARGET URL LOGIC ---
        # Always append /rpc as both custom servers now use this endpoint for proxy calls
        target_url = f"{mcp_base_url.rstrip('/')}/rpc"
        logger.debug(f"Determined target RPC endpoint: {target_url}")
        # --- END MODIFICATION ---

        try:
            self.logger.info(f"Proxying MCP call to {target_url} for tool '{tool_name}' (Req ID: {request_id})")
            self.logger.debug(f"MCP Request Payload: {mcp_request_payload}")

            response = await self.http_client.post(
                target_url, # Use the URL with /rpc appended
            ...
            # --- RESPONSE HANDLING (No changes needed below this point in this function) ---
            try:
    ...
    async def handle_task_send(self, task_id: Optional[str], message: Message, background_tasks: Optional[BackgroundTasks] = None) -> str:
        ...
        if task_id: raise AgentProcessingError(f"MCP proxy agent does not support continuing task {task_id}")
    ...
    async def process_task(self, task_id: str, content: Dict[str, Any]):
        ...
        try:
            ...
            if not target_url:
                raise ConfigurationError(f"No MCP server URL configured for target ID '{input_data.target_mcp_server_id}' or tool prefix '{input_data.tool_name.split('.')}' in MCP_SERVER_MAP.") # Corrected split usage
            ...
            if output_data.success:
                ...
                final_state = TaskStateEnum.FAILED # Keep FAILED if proxy/MCP call failed
            ...
            # 5. Send A2A Response
            response_msg = Message(role="assistant", parts=[DataPart(content=output_data.model_dump(exclude_none=True))])
            try:
    ...
    async def handle_subscribe_request(self, task_id: str) -> AsyncGenerator[A2AEvent, None]:
        ...
        if not self.task_store: raise ConfigurationError("Task store missing.")
        q = asyncio.Queue(); await self.task_store.add_listener(task_id, q); self.logger.info(f"Task {task_id}: Listener queue added.")
        context = await self.task_store.get_task(task_id)
        if context:
            now = datetime.datetime.now(datetime.timezone.utc); status_event = TaskStatusUpdateEvent(taskId=task_id, state=context.current_state, timestamp=now)
        ...
        try:
            ...
            while True:
                try:
                    try: event = await asyncio.wait_for(q.get(), timeout=2.0); event_count += 1; self.logger.info(f"Task {task_id}: Retrieved event #{event_count} from queue: type={type(event).__name__}")
                    except asyncio.TimeoutError:
                        context = await self.task_store.get_task(task_id)
                        if context and context.current_state in [TaskStateEnum.COMPLETED, TaskStateEnum.FAILED, TaskStateEnum.CANCELED]: break
                        continue
                    try: yield event; await asyncio.sleep(0.05)
                    except Exception as yield_err: self.logger.error(f"Task {task_id}: Error during yield: {yield_err}", exc_info=True); break
                except Exception as loop_err: self.logger.error(f"Task {task_id}: Error in main event processing loop: {loop_err}", exc_info=True); break
                context = await self.task_store.get_task(task_id)
                if context and context.current_state in [TaskStateEnum.COMPLETED, TaskStateEnum.FAILED, TaskStateEnum.CANCELED]: break
        except asyncio.CancelledError: self.logger.info(f"Task {task_id}: SSE stream cancelled."); raise
        except Exception as loop_err: self.logger.error(f"Task {task_id}: Error in SSE generator outer loop: {loop_err}", exc_info=True)
        finally: await self.task_store.remove_listener(task_id, q); self.logger.info(f"Task {task_id}: SSE listener removed. Events yielded: {event_count}.")

    async def close(self):
        await self.http_client.aclose()
        self.logger.info("MCP Tool Proxy Agent closed.")
