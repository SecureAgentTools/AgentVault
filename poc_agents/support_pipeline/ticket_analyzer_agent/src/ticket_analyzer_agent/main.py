...
from .agent import TicketAnalyzerAgent
...
try:
    # Use InMemoryTaskStore for PoC
    task_store: BaseTaskStore = InMemoryTaskStore()
    agent_instance = TicketAnalyzerAgent()
    # Assign the store to the agent instance (assuming BaseA2AAgent or subclass needs it)
    if hasattr(agent_instance, 'task_store'):
        agent_instance.task_store = task_store
    else:
        logger.warning("Agent instance does not have a 'task_store' attribute. State management might be limited.")
    ...
    a2a_router = create_a2a_router(
        ...
        tags=["A2A"],
        dependencies=[Depends(lambda: BackgroundTasks())] # Inject BackgroundTasks for agent methods
    )
    ...
    # You might want to exit or have FastAPI return errors on all endpoints
...
@app.get("/", tags=["Status"])
async def read_root():
    return {"message": "Ticket Analyzer Agent running"}

# --- Serve Agent Card ---
# Assumes agent-card.json is in the root of the agent project directory relative to this file
...
logger.info("Ticket Analyzer Agent application initialized.")

# --- Uvicorn Runner (for direct execution, e.g., local testing) ---
# The Dockerfile CMD will typically run this
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8030)) # Default port for this agent
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for Ticket Analyzer Agent on host 0.0.0.0, port {port}")
    # Use the package name for the app string
    uvicorn.run("ticket_analyzer_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False) # reload=False for production/docker
