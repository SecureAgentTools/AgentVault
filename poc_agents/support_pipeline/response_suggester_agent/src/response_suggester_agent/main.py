...
from .agent import ResponseSuggestionAgent
...
# --- FastAPI App Setup ---
app = FastAPI(
    title="Response Suggestion Agent",
    description="Generates draft support responses using context and LLM.",
    version="0.1.0"
)
...
try:
    ...
    agent_instance = ResponseSuggestionAgent()
...
@app.get("/", tags=["Status"])
async def read_root():
    return {"message": "Response Suggestion Agent running"}
...
logger.info("Response Suggestion Agent application initialized.")

# --- Uvicorn Runner (for direct execution) ---
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8033)) # Default port for this agent
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for Response Suggestion Agent on host 0.0.0.0, port {port}")
    uvicorn.run("response_suggester_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
