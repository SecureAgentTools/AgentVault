...
from .agent import KnowledgeBaseSearchAgent
...
# --- FastAPI App Setup ---
app = FastAPI(
    title="Knowledge Base Search Agent",
    description="Searches a knowledge base for relevant articles.",
    version="0.1.0"
)
...
try:
    ...
    agent_instance = KnowledgeBaseSearchAgent()
...
@app.get("/", tags=["Status"])
async def read_root():
    return {"message": "Knowledge Base Search Agent running"}
...
logger.info("Knowledge Base Search Agent application initialized.")

# --- Uvicorn Runner (for direct execution) ---
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8031)) # Default port for this agent
    log_level = os.environ.get("LOG_LEVEL", "info").lower()
    logger.info(f"Starting Uvicorn server for KB Search Agent on host 0.0.0.0, port {port}")
    uvicorn.run("kb_search_agent.main:app", host="0.0.0.0", port=port, log_level=log_level, reload=False)
