"""
Settings module for the Support Pipeline Orchestrator.

Loads settings from .env file or environment variables.
Adapts the e-commerce settings structure.
"""
...
ORCHESTRATOR_SRC_ROOT = CONFIG_DIR.parent # src/support_orchestrator

# Look for .env and config.json in the app root directory (/app in container)
ENV_FILE_PATH = Path("/app/.env")
DEFAULT_CONFIG_PATH = "/app/support_config.json" # Default config file name
...
logger.info(f"Attempting to load .env file for support orchestrator from: {ENV_FILE_PATH}")
...
class Settings(BaseSettings):
    ...
    """
    # Paths and Configuration (REQ-SUP-ORCH-002)
    ...
    AGENTVAULT_REGISTRY_URL: str = Field(
        default="http://localhost:8000", # Default for local run, overridden by .env in Docker
    ...
    LANGCHAIN_PROJECT: Optional[str] = "Support Ticket Pipeline PoC"
...
try:
    ...
    # Configure root logger - force=True ensures it overrides any previous basicConfig
    logging.basicConfig(
        level=log_level_int,
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
        force=True
    )
    logger.info("Support orchestrator settings loaded.")
    ...
    logger.exception(f"CRITICAL: Failed to load support orchestrator settings: {e}")
    # Attempt to load defaults on error to prevent complete failure if possible
    settings = Settings()
